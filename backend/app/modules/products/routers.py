...
async def list_products(db: AsyncSession = Depends(get_db_session)) -> list[ProductRead]:
...
async def create_product(payload: ProductCreate, db: AsyncSession = Depends(get_db_session)) -> ProductRead:
    service = ProductService(ProductRepository(db))
